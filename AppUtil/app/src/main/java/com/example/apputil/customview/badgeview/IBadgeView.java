...
public class IBadgeView {
	...
	public IBadgeView(View view,Context mContext,AttributeSet attrs) {
		...
		mCount=mArray.getString(R.styleable.BadgeView_badge_count);
	 ...
	 * @return
	 */
	public IBadgeView setCount(String count){
		this.mCount=count;
		resetCount();
		return this;
	}
	...
	public IBadgeView setColor(int color){
		mPaint.setColor(Integer.parseInt(mCount));
		mView.invalidate();
		return this;
	}
	private void init() {
	...
	private void resetCount(){
		if(mCount != null && !"".equals(mCount)) {
			...
			//Matcher m = p.matcher(mCount);
			//if(m.matches()){//输入的是数子

			//}else{//输入的是字符串

			//}
			int newCount = Integer.parseInt(mCount);
			if (newCount > 99) {
				mCount = 99 + "+";
			}
